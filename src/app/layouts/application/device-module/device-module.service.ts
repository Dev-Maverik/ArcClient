...
export class DeviceModuleService {
  ...
  getDevices(id?: number): Observable<ResponseModel> {
    ...
    return this.http
      .get<ResponseModel>(`${this.baseUrl}${API_ENDPOINTS.DEVICE.GET}`, {
        params,
      });
      // .pipe(
      //   map((response) => {
  ...
  getStreamProfilesByComponentId(deviceId: number, componentId: number): Observable<ResponseModel> {
      return this.http.get<ResponseModel>(
  ...
  updateStreamProfile(profile: any): Observable<ResponseModel> {
    console.log(profile);
    return this.http.post<ResponseModel>(`${this.baseUrl}${API_ENDPOINTS.STREAMPROFILE.UPDATE}`, profile);
  }

  getDeviecComponentListForSideBar(): Observable<ResponseModel> {
     return this.http.get<ResponseModel>(`${this.baseUrl}${API_ENDPOINTS.DEVICE.GETNAMELIST}`);
  }
}
